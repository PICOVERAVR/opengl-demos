...
GLFWwindow* systemInit() {
	glfwCreateHelper();
	GLFWwindow* window;
	window = glfwCreateWindow(screenWidth, screenHeight, "Demo", nullptr, nullptr);
	if (window == nullptr) {
		glfwTerminate();
...
void systemDestroy(GLFWwindow *w) {
	glfwDestroyWindow(w);
	glfwTerminate();
}
