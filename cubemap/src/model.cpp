...
bool model::loadTexture(GLenum type, string path, GLint binding) { // type of texture, path to texture, and sampler to bind the texture to.
	if (!vboList[0].uv) {
		cerr << "Model likely has no uv coords!" << endl; // check the first mesh, if it doesn't have texture coords it's likely the rest of the model doesn't either
		return false;
	}

	limage::image temp;
	if (!temp.loadImage(type, path, binding)) {
		return false;
	}
...
bool model::loadTextureArray(GLenum type, string* pathList, int numTextures, GLint binding) {
	if (!vboList[0].uv) {
		cerr << "Model likely has no uv coords!" << endl;
		return false;
	}

	limage::image tempCube;
	if (!tempCube.loadImageArray(type, pathList, numTextures, binding)) {
		return false;
	}
